...
metadados_grupos_saude_binario3 <- metadados_grupos_saude_binario2 %>%
  mutate(
    # Pressão arterial
    High_Blood_Pressure = ifelse(!is.na(Systolic) & !is.na(Diastolic) &
                                   (Systolic >= 130 | Diastolic >= 85), 1, 0),
...
metadados_grupos_saude_binario3 <- metadados_grupos_saude_binario3 %>%
  mutate(
    Health_Status_Medicine = case_when(
      Metabolic_Score_Medicine <= 3 ~ "Healthy",
      Metabolic_Score_Medicine >= 4 & Metabolic_Score_Medicine <= 8 ~ "Transition",
      Metabolic_Score_Medicine > 8 ~ "Unhealthy"
    )
  )
...
table(metadados_grupos_saude_binario3$Health_Status_Medicine)




# Criar a tabela de frequência manualmente
grupo_tab <- as.data.frame(table(metadados_grupos_saude_binario3$Health_Status_Medicine))
...
# Gráfico com letras maiores
g_pizza <- ggplot(grupo_tab, aes(x = "", y = n, fill = Health_Status_Medicine)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  theme_void(base_size = 18) +  # Aumenta tamanho da fonte
  geom_text(aes(label = label), position = position_stack(vjust = 0.5), size = 6) +  # Texto maior
