...
set.seed(123)
physeq_rarefied <- rarefy_even_depth(physeq, sample.size = min(sample_sums(physeq)), rngseed = 123, verbose = TRUE)

#UniFrac + PCoA:

# UniFrac ponderado
unifrac_weighted <- UniFrac(physeq_rarefied, weighted = TRUE, normalized = TRUE, parallel = TRUE, fast = TRUE)

# PCoA
ordination_w <- ordinate(physeq_rarefied, method = "PCoA", distance = unifrac_weighted)

# Dataframe para plot
df_pcoa_w <- as.data.frame(ordination_w$vectors)
df_pcoa_w$Sample <- rownames(df_pcoa_w)


library(ggplot2)

ggplot(df_pcoa_w, aes(x = Axis.1, y = Axis.2, label = Sample)) +
  geom_point(size = 3) +
...
samples_s1 <- sample_names(physeq_rarefied)[grepl("^10[01]\\d{2}-F00$", sample_names(physeq_rarefied))]
...
ps_s1 <- prune_samples(sample_names(physeq_rarefied) %in% c(samples_s1, neg_controls_s1), physeq_rarefied)
...
samples_s2 <- sample_names(physeq_rarefied)[grepl("^(10[0-4]|20[0-1]|30[0-2]|40[0-3])\\d{2}_F00(_DNA)?", sample_names(physeq_rarefied))]
...
ps_s2 <- prune_samples(sample_names(physeq_rarefied) %in% c(samples_s2, neg_controls_s2), physeq_rarefied)
...
samples_s3 <- sample_names(physeq_rarefied)[grepl("^40[4-8]\\d{2}_F00(_DNA)?", sample_names(physeq_rarefied))]
...
ps_s3 <- prune_samples(sample_names(physeq_rarefied) %in% c(samples_s3, neg_controls_s3), physeq_rarefied)
...
# Contaminantes comuns (em vermelho):
#  Enterobacteriaceae (Ex.: Enterobacter, Klebsiella, Escherichia-Shigella)
...
#Lactobacillus, mas de espécies inesperadas em fezes humanas (possível contaminação de reagente?)

#Outros destaques:
 # Muitos Firmicutes removidos (e.g., Clostridium, Lactococcus, Streptococcus) – pode ser tanto contaminante quanto microbiota real, mas nesses casos, o decontam detectou padrão de contaminante.

#Presença de Cryptococcus (um fungo) em ASVs removidas → incomum em DNA fecal, fortemente indica contaminação.
...
ggplot(removed_df_real, aes(x = Sample, y = Percent_Removed)) +
  ...
  labs(
    title = "Percentual de Reads Removidos por Amostra (S1, sem controles)",
    x = "Amostra",
...
#Gênero	Comentário
#Pseudomonas	Clássico contaminante de água, reagentes e superfícies.
#Enterobacter, Serratia, Escherichia-Shigella	Enterobactérias comuns como contaminantes de reagentes.
#Staphylococcus, Corynebacterium	Pele humana – comuns em contaminação por manuseio.
#Geobacillus, Bacillus	Extremófilos – contaminantes clássicos de kits de DNA.
#Chloroplast (Punica granatum)	Sequência de cloroplasto vegetal — contaminação de reagente vegetal.
#Thermus, Bilophila, RF39	Aparecem em controles negativos em outros estudos.

#ASVs duvidosas (poderiam ser reais ou contaminantes dependendo do contexto):
#Gênero	Comentário
#Subdoligranulum	Anaeróbio estrito – presente em fezes humanas, mas pode vazar entre wells.
#Prevotella, Blautia	Comuns na microbiota intestinal, mas podem aparecer por cross-contamination.
#Lactobacillus, Streptococcus	Reais, mas podem contaminar reagentes, especialmente em estudos de saliva/fezes.
#Phascolarctobacterium, Dialister, Megasphaera	Gut-related, mas já relatados como possíveis contaminantes em bibliotecas degradadas.
#ASVs plausivelmente reais que merecem atenção:
#Algumas ASVs apontadas como contaminantes também aparecem frequentemente em microbiota intestinal saudável:
  
 # [Eubacterium]_hallii_group

#Lachnospiraceae (diversos)
...
#Essas são suspeitas de falsos positivos na filtragem, principalmente se aparecerem consistentemente em amostras reais com alta abundância e forem removidas apenas por baixa frequência nos negativos.

#Conclusão:
#O decontam acertou bem em muitos casos, removendo táxons clássicos de contaminação.

#Alguns táxons são ambíguos (gut microbes que apareceram pouco nas amostras ou nos negativos).

#Não houve remoção massiva de ASVs altamente confiáveis (ex: Faecalibacterium, Akkermansia, etc.), o que reforça que o threshold 0.5 foi adequado, mas você pode revisar individualmente as ASVs ambíguas com base na prevalência nas amostras reais.
...
ps_s3 <- prune_samples(sample_names(ps) %in% c(samples_s3, neg_controls_s3), physeq_rarefied)
...
# Remover controles do objeto final
amostras_reais <- !sample_names(physeq_descontaminado) %in% c(
  neg_controls_s1, neg_controls_s2, neg_controls_s3
)
physeq_descontaminado <- prune_samples(amostras_reais, physeq_descontaminado)



colnames(sample_data(physeq_descontaminado))


head(sample_data(physeq_descontaminado))

sample_names(physeq_descontaminado)

# Substituir os nomes dos samples no objeto phyloseq
sample_names(physeq_descontaminado) <- gsub("_DNA_S.*", "", sample_names(physeq_descontaminado))
sample_names(physeq_descontaminado) <- gsub("_", ".", sample_names(physeq_descontaminado))
sample_names(physeq_descontaminado) <- gsub("-", ".", sample_names(physeq_descontaminado))
...
setdiff(taxa_names(physeq_final), tree$tip.label) # => se retornar zazio, está ok!


# Lista de outliers
outliers <- c("10161.F00", "40251.F00", "40361.F00", "30181.F00")

# Remover os outliers
physeq_final <- prune_samples(!(sample_names(physeq_descontaminado) %in% outliers), physeq_descontaminado)


# Adicionar a árvore ao objeto phyloseq
phy_tree(physeq_final) <- tree



# Calcular UniFrac ponderado (weighted)
unifrac_weighted <- phyloseq::UniFrac(physeq_final, weighted = TRUE)

# Calcular UniFrac não ponderado (unweighted)
unifrac_unweighted <- phyloseq::UniFrac(physeq_final, weighted = FALSE, normalized = TRUE, parallel = TRUE, fast = TRUE)


#Realizar pcoa
ord_weighted <- ordinate(physeq_final, method = "PCoA", distance = unifrac_weighted)

#Transformar em dataframe e adicionar nomes
df_pcoa_weighted <- as.data.frame(ord_weighted$vectors)
df_pcoa_weighted$Sample <- rownames(df_pcoa_weighted)
