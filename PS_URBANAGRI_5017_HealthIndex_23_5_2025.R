...
pcoa_result2 <- pcoa(dist_matrix2)
pcoa_result2$values$Relative_eig  # proporção explicada por cada eixo

round(pcoa_result2$values$Relative_eig[1] * 100, 2)
...
p4 <- ggplot(pcoa_df2, aes(x = Axis1, y = Axis2, color = HealthGroup)) +
...
pcoa_merged2 <- pcoa_df2 %>%
  left_join(metadados.all.filtrado %>% 
              select(Sample.id, Age, HbA1c, INSULINA, GLICOSE, pulse, COLESTEROL,
                     LDL, HDL, TRIGLICERIDES, TGO, TGP, GGT, IMC, PCR, IMC), 
            by = "Sample.id")
...
variaveis2 <- c("Age", "HbA1c", "GLICOSE", "INSULINA", "pulse", "COLESTEROL", 
               "LDL", "HDL", "TRIGLICERIDES", "TGO", "TGP", "GGT", 
               "IMC", "PCR", "IMC")
...
pcoa_merged3 <- pcoa_df %>%
  left_join(metadados.all.filtrado %>% 
              select(Sample.id, Age, HbA1c, INSULINA, GLICOSE, pulse, HDL, TRIGLICERIDES, TGO, TGP, GGT, IMC, PCR, IMC), 
            by = "Sample.id")
...
# Lista de variáveis
variaveis3 <- c("Age", "GLICOSE", "pulse", "HDL",
                "TRIGLICERIDES", "IMC")
...
metadados_grupos_saude_binario <- metadados.all.filtrado %>%
  mutate(
    ...
    High_Fasting_Glucose = case_when(
      ...
      GLICOSE <= 100 ~ 0,                 # normal
      GLICOSE > 100 & GLICOSE < 126 ~ 1,  # alterada (pré-diabetes)
      GLICOSE >= 126 ~ 2                  # muito alta (diabetes)
    ),
    ...
    TRIG_cat = case_when(
      ...
      TRIGLICERIDES >= 200 & TRIGLICERIDES < 500 ~ 2,
      TRIGLICERIDES >= 500 ~ 3
    ),
    ...
    COL_cat = case_when(
      ...
      COLESTEROL <= 190 ~ 0,
      COLESTEROL > 190 & COLESTEROL <= 240 ~ 1,
      COLESTEROL > 240 ~ 2
    ),
...
vars_grupo_bin <- c("Sample.id", "High_Blood_Pressure", "High_Fasting_Glucose",               
                     "HbA1c_cat",  "IMC_cat","TRIG_cat", "COL_cat","HDL_cat","LDL_cat",                           
                   "GGT_cat")
...
vars_continuas <- c("Systolic", "Diastolic", "Weight", "IMC", 
                    "HbA1c", "COLESTEROL", "LDL", "HDL", "TRIGLICERIDES", 
                     "GGT", "GLICOSE")
...
metadados_binarios_puros <- metadados.all.filtrado %>%
  mutate(
    ...
    # Glicose de jejum > 126 = diabetes
    High_Fasting_Glucose = ifelse(GLICOSE > 126, 1, 0),
    ...
    High_Trig = ifelse(TRIGLICERIDES >= 200, 1, 0),
    
    # Colesterol total > 200 = alterado
    High_Col = ifelse(COLESTEROL > 200, 1, 0),
  ...
  # Selecionar apenas colunas binárias e ID
...
variaveis_continuas <- c("GLICOSE", "HbA1c", "COLESTEROL", "LDL", "HDL",
                         "TRIGLICERIDES", "TGO", "TGP", "GGT", "IMC")
...
metadados_grupos_saude_binario <- metadados_grupos_saude_binario %>%
  mutate(
    Metabolic_Score = rowSums(across(c(
      ...
      IMC_cat, TRIG_cat, COL_cat, HDL_cat, LDL_cat, GGT_cat
    ), ~replace_na(., 0)))
  )
...
ggplot(metadados_grupos_saude_binario, aes(x = Health_Status, y = GLICOSE, fill = Health_Status)) +
  geom_boxplot() +
  labs(title = "Glicemia em jejum por Grupo de Saúde", x = "", y = "Glicose (mg/dL)") +
  theme_minimal(base_size = 14)

#Ver se é significante
kruskal.test(GLICOSE ~ Health_Status, data = metadados_grupos_saude_binario)

install.packages("FSA")  # se ainda não tiver
library(FSA)

# Teste de Dunn com correção de Bonferroni
dunnTest(GLICOSE ~ Health_Status, data = metadados_grupos_saude_binario, method = "bonferroni")
...
ggplot(metadados_grupos_saude_binario, aes(x = Health_Status, y = GLICOSE, fill = Health_Status)) +
  ...
  labs(title = "Glicemia em jejum por Grupo de Saúde", x = "", y = "Glicose (mg/dL)") +
  theme_minimal(base_size = 14) +
  stat_compare_means(method = "kruskal.test", label.y = max(metadados_grupos_saude_binario$GLICOSE, na.rm = TRUE) + 5)
...
ggplot(metadados_grupos_saude_binario, aes(x = Health_Status, y = GLICOSE, fill = Health_Status)) +
  geom_boxplot() +
  stat_compare_means(method = "kruskal.test", label.y = max(metadados_grupos_saude_binario$GLICOSE, na.rm = TRUE) + 10) +  # Teste global
  ...
  label.y = c(
    max(metadados_grupos_saude_binario$GLICOSE, na.rm = TRUE) + 30,
    max(metadados_grupos_saude_binario$GLICOSE, na.rm = TRUE) + 20,
    max(metadados_grupos_saude_binario$GLICOSE, na.rm = TRUE) + 10)
  ) +
  labs(title = "Glicemia em jejum por Grupo de Saúde", x = "", y = "Glicose (mg/dL)") +
  theme_minimal(base_size = 14)
...
g_pizza <- ggplot(grupo_tab, aes(x = "", y = n, fill = Health_Status)) +
  ...
  labs(title = "Metabolic Risk Distribution") +
...
metadados_binarios_puros <- metadados.all.filtrado %>%
  mutate(
    ...
    # IMC categorizado por idade
    IMC_cat = case_when(
      Age >= 60 & IMC < 22 ~ 0,
      Age >= 60 & IMC >= 22 & IMC < 27 ~ 0,
      Age >= 60 & IMC >= 27 & IMC < 30 ~ 0,
      Age >= 60 & IMC >= 30 & IMC < 35 ~ 1,
      Age >= 60 & IMC >= 35 ~ 1,
      Age < 60 & IMC < 18.5 ~ 1,
      Age < 60 & IMC >= 18.5 & IMC < 25 ~ 0,
      Age < 60 & IMC >= 25 & IMC < 30 ~ 0,
      Age < 60 & IMC >= 30 & IMC < 35 ~ 1,
    ...
    COL_cat = case_when(
      COLESTEROL <= 200 ~ 0,
      COLESTEROL > 200 & COLESTEROL <= 240 ~ 1,
      COLESTEROL > 240 ~ 1,
      TRUE ~ NA_integer_
    ),
  ...
  mutate(across(
    c(High_Blood_Pressure, High_Fasting_Glucose, HbA1c_cat, IMC_cat, TRIG_cat,
...
colunas_binarias <- c("High_Blood_Pressure",              
                      "High_Fasting_Glucose",              "HbA1c_cat" ,                         "IMC_cat" ,                          
...
for (cit in citocinas) {
  dados <- metadados.metabolic.score[, c("Health_Status", cit)]
...
z_inflam <- scale(metadados.metabolic.score[, inflamatorias])
z_antiinflam <- scale(metadados.metabolic.score[, antiinflamatorias])

# Calcular o escore médio por indivíduo
metadados.metabolic.score$Inflam_Score <- rowMeans(z_inflam, na.rm = TRUE)
metadados.metabolic.score$AntiInflam_Score <- rowMeans(z_antiinflam, na.rm = TRUE)

#=====> Comparar os escores entre os grupos


# Kruskal-Wallis para cada escore
kruskal.test(Inflam_Score ~ Health_Status, data = metadados.metabolic.score)
kruskal.test(AntiInflam_Score ~ Health_Status, data = metadados.metabolic.score)

#=======> Boxplots com interpretação

library(ggplot2)

ggplot(metadados.metabolic.score, aes(x = Health_Status, y = Inflam_Score, fill = Health_Status)) +
  geom_boxplot() +
  labs(title = "Escore inflamatório (z-score médio)", y = "Inflam_Score") +
  theme_minimal(base_size = 14) +
  scale_fill_brewer(palette = "Spectral")

ggplot(metadados.metabolic.score, aes(x = Health_Status, y = AntiInflam_Score, fill = Health_Status)) +
  geom_boxplot() +
...
# Corrigir lista de variáveis (sem duplicata de IMC)
metabolicos <- unique(c("HbA1c", "GLICOSE", "INSULINA", "COLESTEROL", 
                        "LDL", "HDL", "TRIGLICERIDES", "TGO", "TGP", "GGT", 
                        "IMC", "PCR"))
...
for (i in citocinas) {
  for (j in metabolicos) {
    x <- metadados.metabolic.score[[i]]
    y <- metadados.metabolic.score[[j]]
